class WeatherDashboard {
    ...
    async syncData(fetchFunction) {
        const promises = [...this.#cities].map(city =>
            fetchFunction(city)
                ...
                .catch(() => {}) // ignore failed fetch (e.g., Atlantis)
        );
    ...
    getHottestCity() {
        // check the current size of the weather data
        if (this.#weatherData.size === 0){
            return "Empty Weather data";
        }

        // set the hottest city to null
        let hot_city = null;
        let current_high = -Infinity;
        for (const [city, weather] of this.#weatherData.entries()){
            if ( weather.temp > current_high){
                current_high = weather.temp;
                hot_city = city;
            };
        };

        return `${hot_city} with temperature of ${current_high} degrees celcius`;
